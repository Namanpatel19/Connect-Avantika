...
class MainRepository {
    ...
    suspend fun joinClub(clubId: String, studentId: String): Result<Unit> = withContext(Dispatchers.IO) {
        try {
            ...
            if (existing != null) return@withContext Result.failure(Exception("Already applied"))
    ...
    suspend fun uploadEventBanner(file: File): Result<String> = withContext(Dispatchers.IO) { try { val path = "banners/event_${System.currentTimeMillis()}.${file.extension}"; adminStorage.from("event_banners").upload(path, file.readBytes()) { upsert = true }; val publicUrl = adminStorage.from("event_banners").publicUrl(path); Result.success(publicUrl) } catch (e: Exception) { Result.failure(e) } }
    ...
    suspend fun deleteFaculty(userId: String): Result<Unit> = withContext(Dispatchers.IO) { try { adminDb.from("faculty").delete { filter { eq("user_id", userId) } }; adminDb.from("users").delete { filter { eq("id", userId) } }; try { adminAuth.deleteUser(userId) } catch (e: Exception) {}; Result.success(Unit) } catch (e: Exception) { Result.failure(e) } }
    ...
    suspend fun uploadProfileImage(userId: String, file: File, role: String): Result<String> = withContext(Dispatchers.IO) { try { val path = "profiles/${userId}_${System.currentTimeMillis()}.${file.extension}"; adminStorage.from("profiles").upload(path, file.readBytes()) { upsert = true }; val publicUrl = adminStorage.from("profiles").publicUrl(path); if (role == "student") adminDb.from("students").update({ set("photo_url", publicUrl) }) { filter { eq("user_id", userId) } } else adminDb.from("faculty").update({ set("photo_url", publicUrl) }) { filter { eq("user_id", userId) } }; Result.success(publicUrl) } catch (e: Exception) { Result.failure(e) } }
